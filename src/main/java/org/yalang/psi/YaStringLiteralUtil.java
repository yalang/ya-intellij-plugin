 ...
 */
public class YaStringLiteralUtil {
  /**
  ...
  private static final Logger LOG = Logger.getInstance(YaStringLiteralUtil.class);

  private YaStringLiteralUtil() {
  }
  ...
  public static boolean isStringLiteralToken(@NotNull String text) {
    final YaLexerAdapter lexer = new YaLexerAdapter();
    lexer.start(text);
    return YaTokenTypes.STRING_NODES.contains(lexer.getTokenType()) && lexer.getTokenEnd() == lexer.getBufferEnd() ||
            YaTokenTypes.FSTRING_START == lexer.getTokenType();
  }
