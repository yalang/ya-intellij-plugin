 ...
 */
public class YaLangStringLiteralUtil {
  /**
  ...
  private static final Logger LOG = Logger.getInstance(YaLangStringLiteralUtil.class);

  private YaLangStringLiteralUtil() {
  }
  ...
  public static boolean isStringLiteralToken(@NotNull String text) {
    final YaLangLexerAdapter lexer = new YaLangLexerAdapter();
    lexer.start(text);
    return YaLangTokenTypes.STRING_NODES.contains(lexer.getTokenType()) && lexer.getTokenEnd() == lexer.getBufferEnd() ||
            YaLangTokenTypes.FSTRING_START == lexer.getTokenType();
  }
...
//    if (element instanceof YaLangStringLiteralExpression) {
//      final List<TextRange> ranges = ((YaLangStringLiteralExpression)element).getStringValueTextRanges();
//      return ranges.get(0);
//    }
//    else {
...
//    }
  ...
  @Nullable
  public static String getText(@Nullable NavigatablePsiElement ex) {
//    if (ex == null) {
//      return null;
//    }
//    else {
